/****************************************************/
// NetRad firmware
// Heavily based on original firmware written by Shigeru 
// Kobayashi (aka kotobuki). 
//
/****************************************************/
...
void cmdSetFeedID(int arg_cnt, char **args)
{
  ...
  eeprom_write_block((byte *)&dev, 0, sizeof(device_t));
  printf_P(PSTR("Feed ID set to %06u.\n"), dev.feedID);
}
...
void cmdStat(int arg_cnt, char **args)
{
  ...
  printf_P(PSTR("Device Feed ID \t= %06u.\n"), dev.feedID);
  printf_P(PSTR("Device ID \t= %s.\n"), dev.devID);
}
